...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT VEHICLES.*,CARBRANDS.BrandName,CARBRANDS.id as bid from VEHICLES join CARBRANDS on CARBRANDS.id=VEHICLES.VehiclesBrand where VEHICLES.id=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
...
<?php $ret="select id,BrandName from CARBRANDS";
